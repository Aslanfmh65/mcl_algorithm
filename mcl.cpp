...
namespace plt = matplotlibcpp; // Graph plot
using namespace std;
...
// Random Generators, needed for gen_real_random() function
...
public:
   ...
   double x, y, orient; // robot's pose
   ...
   Robot()
   {
    ...
    forward_noise = 0.0; //noise of the forward movement initiated
    turn_noise = 0.0; //noise of the turn movement initiated
    sense_noise = 0.0; // noise of the sense movement initiated
   }

    void set(double new_x, double new_y, double new_orient)
    {
         // Set robot new position and orientation, and make sure they are inside the world
    ...
    vector<double> sense()
    {
        ...
        int size_vector = sizeof(landmarks)/sizeof(landmarks[0]);
        vector<double> z(size_vector);
        double dist;
...
double gen_real_random()
{
   // Generate real randome between 0 and 1
   uniform_real_distribution<double> real_dist(0.0, 1.0); 
   return real_dist(gen);
}

double mod(double first_term, double second_term) // why can't we use first_term % second_term??????????????????????????????????????
{
   // Compute the modulus
...
int main ()
{
    Robot myrobot; // Initialize a robot object from Robot class
